---
id: aws-rds
slug: /installation/installation-methods/aws-rds
title: AWS RDS
---
...
<Tabs>
  <TabItem value="cloudformation" label="CloudFormation" default>
   ...
   1. Create read-only user "releem" in MySQL using the [instruction](/supported-databases/mysql/required-permissions). To enable Automatic SQL Query Optimization please add [Additional Permissions](/supported-databases/mysql/required-permissions#additional-database-permissions-required).
...
3. Error 1045 (28000): Access denied for user 'releem'@'' (using password: YES)
   - Check password for Releem User that you created according to this [guide](/supported-databases/mysql/required-permissions). And reinstall Releem Agent.
...
6. Error 1142 (42000): SELECT command denied to user 'releem'@'[serverIP]' for table 'events_statements_history'
   - Additional database permissions are required. Follow this guide to add them: [Additional Permissions](/supported-databases/mysql/required-permissions#additional-database-permissions-required)
