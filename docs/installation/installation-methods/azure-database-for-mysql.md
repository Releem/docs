...
slug: /installation/installation-methods/azure-database-for-mysql
...
1. Create the `releem` MySQL read-only  user using the [MySQL permissions guide](/supported-databases/mysql/required-permissions). Add [Additional Permissions](/supported-databases/mysql/required-permissions#additional-database-permissions-required) if SQL Query Optimization will be enabled.
