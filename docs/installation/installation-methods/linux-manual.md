---
id: linux-manual
slug: /installation/installation-methods/linux-manual
title: "Linux (Manual)"
---
...
:::note CloudLinux
If you install Releem Agent on CloudLinux, review the [CloudLinux-specific steps](/get-started/troubleshoot-releem-agent#cloudlinux) for CageFS and MySQLGovernor after installation.
:::

1. Create read-only user "releem" in MySQL using the [instruction](/supported-databases/mysql/required-permissions).
   To enable Automatic SQL Query Optimization please add [Additional Permissions](/supported-databases/mysql/required-permissions#additional-database-permissions-required).
2. Run Installation command as a root user on the server:
   ...
   **Parameters:**
   ...
   - `RELEEM_DB_MEMORY_LIMIT` - Change parameter in case there are other software installed on the server. Default value is 0 means use all memory. [Learn more](/recommendations/configuration-tuning/limit-mysql-memory)
