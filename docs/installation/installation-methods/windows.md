---
id: windows
slug: /installation/installation-methods/windows
title: "Windows"
---
...
- `RELEEM_DB_MEMORY_LIMIT` - change this parameter in case there is other software installed on the server. Default value `0` means use all memory. [Learn more](/recommendations/configuration-tuning/limit-mysql-memory)
...
1. Create read-only MySQL user "releem" using this [guide](/supported-databases/mysql/required-permissions).
...
4. Create a file `C:\ProgramData\ReleemAgent\releem.conf` with the following contents:
   ...
   For a full list of configuration settings, please refer to the [Releem Agent Configuration Settings](/installation/manage-the-releem-agent/configuration).
