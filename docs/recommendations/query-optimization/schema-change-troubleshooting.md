...
slug: /recommendations/query-optimization/schema-change-troubleshooting
...
Some dashboard views show a generic title such as `Schema changes execution failed` and then append the detailed error sent by the agent. Match the detailed text against the tables below. For logs, see [How to Check Releem Agent Logs](/installation/manage-the-releem-agent/logs).

For setup requirements, see [Automatic Schema Changes](/recommendations/query-optimization/automatic-schema-changes).
...
`pt-online-schema-change` usually needs privileges such as `SELECT`, `INSERT`, `DROP`, `RELOAD`, `SHOW VIEW`, and `TRIGGER`, plus the version-specific replication privileges documented in [Automatic Schema Changes](/recommendations/query-optimization/automatic-schema-changes#grant-permissions-for-pt-online-schema-change). Grant `SUPER` only if the tool fails with an error that explicitly requires it.
