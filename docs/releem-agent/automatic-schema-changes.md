...
The same **MySQL user** the agent already uses for monitoring must be able to run the statements Releem will apply (for example `ALTER` and related DDL on the relevant databases).

#### To allow table ALTERs and New indexes on a specific database (`airport` in this case)
```sql
...
#### For schema changes dry-runs (note this only affects the test database)
```sql
...
That user must be allowed to create the **scratch database** used to validate online DDL (see `online_ddl_test_schema` below). The agent creates it if it does not exist.
